...
class Duels(commands.Cog):
    ...
    async def challenge(self, interaction: discord.Interaction, opponent: discord.Member, challenger: discord.Member = None, skip_mentions: bool = False) -> None:
        ...
        view.add_item(DuelButton(self, label=f"{challenger.display_name} Wins", style=discord.ButtonStyle.green, custom_id=f"duelWin_{channel.id}_{challenger.id}_{opponent.id}_challenger"))
        view.add_item(DuelButton(self, label=f"{opponent.display_name} Wins", style=discord.ButtonStyle.green, custom_id=f"duelWin_{channel.id}_{challenger.id}_{opponent.id}_opponent"))
        view.add_item(DuelButton(self, label="Refuse challenge (-2pts)", style=discord.ButtonStyle.red, custom_id=f"duelRefused_{channel.id}_{challenger.id}_{opponent.id}_none"))
        view.add_item(DuelButton(self, label="Cancel challenge (only for legitimate use, no score change)", style=discord.ButtonStyle.blurple, custom_id=f"duelCancelled_{channel.id}_{challenger.id}_{opponent.id}_none"))
    ...
    async def buttonHandling(self, button: DuelButton, interaction: discord.Interaction) -> None:
